...
export class GeoLibreMap implements OnDestroy {
  ...
  origin = computed(() => {
    ...
    try {
      const base = typeof window !== 'undefined' ? window.location.origin : 'http://localhost';
      return new URL(this.runtimeUrl(), base).origin;
    } catch {
      return '';
    }
  });
  ...
  safeUrl = computed<SafeResourceUrl | null>(() => {
    const sanitized = this.sanitizer.sanitize(SecurityContext.URL, this.runtimeUrl());
    if (!sanitized) {
      return null;
    }
  ...
  constructor() {
    effect(() => {
      ...
      void this.syncMap(origin, commands, focusCommands).catch((error: unknown) => {
  ...
  private async syncMap(origin: string, commands: Gn4MapCommand[], focusCommands: Gn4MapCommand[]) {
    const iframeEl = this.geolibreIframe()?.nativeElement;
    if (!iframeEl) {
      return;
    }

    const connectionKey = `${this.runtimeUrl()}|${origin}`;
  ...
  private buildDataUrl(cmd: Gn4MapCommand, layerType: Gn4MapCommand['type']): string {
    ...
    try {
      const base = typeof window !== 'undefined' ? window.location.origin : 'http://localhost';
      const url = new URL(this.runtimeUrl(), base);
